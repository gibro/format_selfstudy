...
const renderPathContinueButton = (main, config, pathOutline) => {
    if (document.querySelector('.format-selfstudy-pathcontinue')) {
        return;
    }
    ...
    wrapper.className = 'format-selfstudy-pathcontinue';
    wrapper.setAttribute('aria-label', config.continueLabel || 'Weiterlernen');

    const link = document.createElement('a');
    link.className = 'btn btn-primary format-selfstudy-pathcontinue-button';
    link.href = target.url;
    link.textContent = target.completion ?
        (config.completionLabel || 'Kursübersicht') :
        (config.continueLabel || 'Weiterlernen');
    wrapper.appendChild(link);

    if (target.title) {
        const label = document.createElement('span');
        label.className = 'format-selfstudy-pathcontinue-target';
