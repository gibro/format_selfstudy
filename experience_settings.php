...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $learnerview = optional_param('learnerview', 'base', PARAM_TEXT);
    ...
    foreach ($components as $index => $component) {
        ...
        $configformclass = format_selfstudy_experience_settings_get_config_form_class($component);
        if ($configformclass && method_exists($configformclass, 'get_config_from_request')) {
            $config = $configformclass::get_config_from_request('experience_config_' . $index, $config);
        }
    ...
    $format->update_course_format_options(['learnerview' => $learnerview]);
...
if (!$entries) {
    ...
    echo html_writer::start_tag('section', ['class' => 'format-selfstudy-learnerview-selection']);
    echo html_writer::tag('h3', get_string('learnerviewselection', 'format_selfstudy'));
    echo html_writer::div(get_string('learnerviewselection_help', 'format_selfstudy'), 'text-muted');
    echo html_writer::div(
        html_writer::label(
            html_writer::empty_tag('input', [
                ...
                'name' => 'learnerview',
                'value' => 'base',
            ] + ($selectedlearnerview === 'base' ? ['checked' => 'checked'] : [])) .
            ' ' . get_string('learnerviewbase', 'format_selfstudy'),
            ''
        ),
        'form-check'
    );
        ...
        echo html_writer::div(
            html_writer::label(
                html_writer::empty_tag('input', [
                    ...
                    'name' => 'learnerview',
                    'value' => $viewvalue,
                ] + ($selectedlearnerview === $viewvalue ? ['checked' => 'checked'] : [])) .
    ...
    foreach (array_values($entries) as $index => $entry) {
        ...
        if ($selectedlearnerview === 'experience:' . $entry->component && !$entry->missing) {
        ...
        echo html_writer::tag('td',
            ...
            ($entry->description !== '' ? html_writer::div(format_text($entry->description, FORMAT_PLAIN),
                'text-muted') : '') .
            format_selfstudy_experience_settings_render_config($course, $entry, $index)
        );
...
/**
 * Renders experience-specific config controls when provided by the subplugin.
 *
 ...
 * @return string
 */
function format_selfstudy_experience_settings_render_config(stdClass $course, stdClass $entry, int $index): string {
    if ($entry->missing || $entry->configformclass === '' || !class_exists($entry->configformclass) ||
            !method_exists($entry->configformclass, 'render')) {
        return '';
    }

    return $entry->configformclass::render($course, $entry->config, 'experience_config_' . $index);
}
 ...
 * @return string
 */
function format_selfstudy_experience_settings_get_config_form_class(string $component): string {
